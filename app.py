...
def simulate_graphene_cvd(T_C, P_torr, ch4_h2, t_min, o2_ppm, method_key, feed_key, sub_key, wafer_diam_cm):
    ...
    # Growth rate: directly calibrated empirical model
    # Literature: at 1050C, 0.5 Torr, CH4/H2=0.01 on Cu -> G ~ 1 um/s
    # G = G0 * exp(-Ea/kT) * (CH4/H2)^0.5 * P^0.3
    Ea_growth = 2.0  # eV, effective activation energy for growth
    G0 = 5e8  # um/s, pre-exponential (calibrated)
    G_attach_um_s = G0 * np.exp(-Ea_growth / (kB_eV * T_K)) * np.sqrt(ch4_h2) * P_torr**0.3
    P_H2 = P_torr * (1 - ch4_h2) / (1 + ch4_h2)
    # H2 etching: much slower than growth at normal conditions
    ...
    ID_IG = max(0.005, min(3.0, gb + o2_t + anneal + rate_t + method_t))
...
with tab1:
    ...
    with col_ctrl:
        ...
        T = st.slider("Temperature (C)", cvd["T_range"][0], cvd["T_range"][1], int(np.mean(cvd["T_range"])), 10)
        P = st.slider("Pressure (Torr)", 0.1, min(100.0, cvd["P_range"][1]), 0.5, 0.1)
